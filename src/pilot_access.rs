...
pub fn expand(node: &DeriveInput) -> Result<TokenStream> {
    ...
    Ok(quote! {
        impl crate::pilot::bindings::PilotAccess for #struct_name {
            ...
            const VARIABLES: &'static [pilot_sys::var::VariableInfo] = &[#(#variables),*];

            fn plc_varnumber_to_variable(&self, number: u16) -> Option<&dyn pilot_sys::var::MemVar> {
                match number {
                    #(#plc_varnumber_to_variable_inner_fields)*
                    _ => None,
                }
