...
pub fn expand(node: &DeriveInput) -> Result<TokenStream> {
    ...
    Ok(quote! {
        impl pilot_sys::var::PilotBindings for #struct_name {
            ...
            match attribute.path.get_ident().map(|i| i.to_string()).as_deref() {
                Some("bind_read") => {
                    ...
                    set_from_pilot_bindings.push(quote_spanned! {field.ty.span()=>
                        VarProps::set(&self.#field_name, #filtered);
                    });
                }
                Some("bind_write") => {
                    bind_attribute_found = true;
                    ...
                    let value = quote_spanned!(field.ty.span()=> VarProps::get(&self.#field_name) );
                    let filtered = match parsed.filter {
                        Some(filter) => quote!(#filter ( #value )),
                        None => quote!(#value),
                    };
                ...
                Some("bind_nested") => {
                    ...
                    set_from_pilot_bindings.push(quote_spanned! {field.ty.span()=>
                        #check_trait
                        pilot_sys::var::PilotBindings::set_from_pilot_bindings(&self.#field_name, &#plc_mem);
                    });
                    write_to_pilot_bindings.push(quote_spanned! {field.ty.span()=>
                        #check_trait
                        pilot_sys::var::PilotBindings::write_to_pilot_bindings(&self.#field_name, &mut #plc_mem);
                    });
                }
                Some("bind_ignore") => {
