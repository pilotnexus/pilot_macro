...
pub fn expand(item: &ItemStatic) -> Result<TokenStream> {
    ...
    Ok(quote_spanned!(static_ty.span()=>
        #[no_mangle]
        static PLC_VARIABLES: pilot_sys::var::VariableInfo = pilot_sys::var::VariableInfo {
        ...
        unsafe fn plc_varnumber_to_variable(number: u16) -> Option<&'static dyn pilot_sys::var::MemVar> {
           crate::pilot::bindings::PilotAccess::plc_varnumber_to_variable(&#static_varname, number)
        }

        #[no_mangle]
        unsafe fn plc_mem_to_var() {
            let plc_mem: &crate::pilot::bindings::plc_dev_t = _get_plc_mem_devices_struct();
            pilot_sys::var::PilotBindings::set_from_pilot_bindings(&#static_varname, plc_mem);
        }

        #[no_mangle]
        unsafe fn plc_var_to_mem() {
            let plc_mem: &mut crate::pilot::bindings::plc_dev_t = _get_plc_mem_devices_struct();
            pilot_sys::var::PilotBindings::write_to_pilot_bindings(&#static_varname, plc_mem);
        }
